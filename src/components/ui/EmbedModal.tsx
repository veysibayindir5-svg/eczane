...
export const EmbedModal: React.FC<Props> = ({ isOpen, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-[2.5rem] w-full max-w-4xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col md:flex-row animate-in fade-in zoom-in duration-300">

                {/* Left Side: Preview */}
                <div className="hidden md:flex flex-col bg-slate-50 border-r border-slate-100 w-1/2 p-8">
                    ...
                    <div className="flex-grow flex items-center justify-center overflow-auto p-4">
                        <div style={{ width: previewSize }} className="transition-all duration-300 bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-200">
                            <iframe
                                src="/embed"
                ...
                <div className="flex-grow p-8 md:p-10 flex flex-col">
                    ...
                    <div className="mb-8">
                        ...
                        <h2 className="text-3xl font-extrabold text-slate-900 mb-3 tracking-tight">Sitenize Ekleyin</h2>
                    ...
                    <div className="space-y-6 flex-grow">
                        <div className="relative group">
                            <div className="absolute -top-3 left-4 bg-white px-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none">
                                HTML Embed Kodu
                            </div>
                            <div className="bg-slate-900 rounded-2xl p-6 pt-8 font-mono text-sm text-slate-300 overflow-x-auto border border-slate-800 shadow-inner group-hover:border-brand-500 transition-colors">
                                <code className="whitespace-pre">{embedCode}</code>
                            </div>
                            <button
                                onClick={handleCopy}
                                className="absolute top-4 right-4 bg-white/10 hover:bg-white/20 text-white p-2.5 rounded-xl backdrop-blur-md transition-all active:scale-90"
                                title="Kopyala"
                            >
                    ...
                    <div className="mt-10 pt-8 border-t border-slate-100 flex items-center justify-between">
                        <button
                            onClick={handleCopy}
                            className={`flex items-center justify-center gap-3 px-8 py-4 rounded-2xl font-bold text-lg transition-all active:scale-95 shadow-lg ${copied ? 'bg-emerald-500 text-white shadow-emerald-200' : 'bg-brand-600 text-white shadow-brand-200 hover:bg-brand-700'}`}
                        >
                        ...
                        <a
                            href="/embed"
                            target="_blank"
                            className="hidden sm:flex items-center gap-2 text-slate-400 hover:text-brand-600 font-bold text-sm transition"
                        >
